...
class REFUGE(Dataset):
    ...
    def __getitem__(self, index):
        ...
        multi_rater_cup_path = [os.path.join(subfolder, name + '_seg_cup_' + str(i) + '.png') for i in range(1, 8)]#7 个标注者分别标注的 GT
        ...
        if self.prompt == 'click':
            point_label, pt = random_click(np.array(np.mean(np.stack(multi_rater_cup_np), axis=0)) / 255, point_label,num_points=3)
            # point_label, pt_disc = random_click(np.array(np.mean(np.stack(multi_rater_disc_np), axis=0)) / 255, point_label,num_points=1)
        else:
